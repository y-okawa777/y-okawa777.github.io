...
privacy/contact.js    … ポリシーページ用のフォーム送信+アドレスコピー処理(文言はHTML側のdata-*属性で指定)
...
- 問い合わせフォームは **Formspree**(エンドポイント: `https://formspree.io/f/xkodrbdb`、無料プランは月50件まで)経由で `app.yutech@gmail.com` に届く。送信はJSでAJAX化しており、失敗時は代替のアドレス表示+コピーUIが下にある。届け先メールを変えたい場合はFormspreeのダッシュボード側で設定変更する(コード側の変更は不要)。
- フォームは **トップページ(main.js + i18n.js)とポリシーページ(privacy/contact.js + data-*属性)の計3箇所** にある。フォームの仕様を変える時は index.html / privacy/ja.html / privacy/en.html すべてを揃えること。
